...
CSE.xhttp = new XMLHttpRequest();
CSE.xhttp.open("GET", "http://localhost:3000/points", true); 
CSE.xhttp.send();

//Draw point
CSE.xhttp.onreadystatechange = function ()
    {
        if(CSE.xhttp.readyState == 4 && CSE.xhttp.status == 200)
        {
            CSE.drawPoints(JSON.parse(CSE.xhttp.responseText));   
        }
    }

CSE.drawPoints = function(pointsToDraw)
    {   console.log(pointsToDraw)
        pointsToDraw.forEach(function(existing_point)
            {
                L.circleMarker([existing_point.lat, existing_point.lng], CSE.markers.unsynced)
                 .addTo(CSE.map);
            });
    }